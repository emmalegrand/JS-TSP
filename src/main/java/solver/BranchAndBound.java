...
public class BranchAndBound<S extends State> {
...
//                    onSolution.accept(p);
                ...
                nb_nodes++;
                for (Integer child : model.next_variables(p.state,bnbHeuristic)) {
                    Transition<S> t = model.transition(p.state, child);
                    double h = model.lowerBound(t.getSuccessor());
                    double gval = p.val;
                    double ghval = max(gval + t.getValue() + h, p.h);
                    if ( ghval < upperBound) {
                        openNode.add(t.getSuccessor(), ghval,gval + t.getValue());
                    }
                }
            }
        }
