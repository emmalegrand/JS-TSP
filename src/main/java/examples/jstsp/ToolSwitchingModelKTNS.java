...
public class ToolSwitchingModelKTNS implements Model<ToolSwitchingKTNSState> {
    ...
    ToolSwitchingBruteForce bf;

    public ToolSwitchingModelKTNS(ToolSwitchingInstance instance, double[][] distanceMatrix) {
        ...
        this.bf = new ToolSwitchingBruteForce(instance);
    }

    @Override
    public boolean isBaseCase(ToolSwitchingKTNSState state) {
        return state.remaining == 0;
    }

    @Override
    public double getBaseCaseValue(ToolSwitchingKTNSState state) {
        return 0;
    }

    @Override
    public boolean isGoal(ToolSwitchingKTNSState state) {
        return state.remaining == 0;
    }

    @Override
    public ToolSwitchingKTNSState initial_state() {
        long remaining = 0L;
        for (int i = 1; i < this.instance.n; i++) {
            remaining |= 1L << i;
        }
        return new ToolSwitchingKTNSState(new ArrayList<>(this.instance.n),0, remaining);
    }

    @Override
    public Transition<ToolSwitchingKTNSState> transition(ToolSwitchingKTNSState source, int decision) {
        ToolSwitchingKTNSState state = new ToolSwitchingKTNSState((ArrayList<Integer>) source.seq.clone(),decision,source.remaining & ~(1L << decision));
        int cost = bf.evaluate(state.seq.stream().mapToInt(i -> i).toArray());
        return new Transition<>(state,decision,cost);
    }

    @Override
    public List<Integer> next_variables(ToolSwitchingKTNSState state, boolean bnb_heuristic) {
    ...
    @Override
    public double lowerBound(ToolSwitchingKTNSState state) {
    ...
    double lowerBoundL2(ToolSwitchingKTNSState state) {
    ...
    double easyLowerBound(ToolSwitchingKTNSState state) {
        long toolsneeded = 0L;
        long toolactive = 0L;
        for (int i = 0; i < this.instance.n; i++) {
            if ((state.remaining & (1L << i)) == 1L << i) {
    ...
    class SortByMostSimilar implements Comparator<Integer> {
        private ToolSwitchingKTNSState current;
        public SortByMostSimilar(ToolSwitchingKTNSState current){
